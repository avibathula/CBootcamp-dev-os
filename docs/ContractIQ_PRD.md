...
1. User types a question such as "What happens if I breach the NDA?", "Is there an auto-renewal clause?", or a follow-up like "what have I asked you so far?"
2. The backend classifies the question (contract / history / both) and retrieves only the context that classification calls for — the extracted contract text for contract/both questions, recent conversation turns for all three — then sends that to OpenAI with a system prompt matched to the classification, instructing it to answer only from the retrieved context
3. The response appears in a chat UI (user messages right-aligned, AI responses left-aligned) with a source badge showing what it was grounded in
4. Each AI response cites its source inline — "[Page X]" for contract facts (clickable, links to the relevant page in the PDF viewer) and "[From conversation]" for facts drawn from earlier in the chat
5. Conversation is saved in `chat_messages` table, linked to `chat_sessions`, linked to the contract; each assistant message stores its classification alongside its content
...
| FR-08 | The chat interface must classify each question (contract / history / both), retrieve only the matching context, and send it to OpenAI with a matching system prompt, displaying the grounded, attributed response | P1 | Classification-specific system prompts enforce document-only or conversation-only answers as appropriate; responses cite `[Page X]` and/or `[From conversation]` |
| FR-09 | All chat messages must be saved to Supabase in real-time with role (user / assistant), timestamp, and — for assistant messages — the classification that produced them | P1 | Linked to chat_sessions → contracts → users; `chat_messages.source_type` persists attribution across reload |
...
| Contract Chat Agent | User question, classified as contract/history/both, + the matching context (contract text and/or last 10–20 conversation turns) | Plain-English answer attributed with `[Page X]` and/or `[From conversation]` | Suggests, human decides | Always: AI cannot take actions on the contract; it only answers questions |
...
- **Chat grounding (RAG-style, classification-gated):** The chat route classifies each question first (`contract` / `history` / `both`) and only then retrieves context — contract text is included for `contract`/`both` questions and deliberately **excluded** for `history` questions, so a memory-only question can't lean on the document. The system prompt is classification-specific and every fact is attributed inline: `[Page X]` for contract facts, `[From conversation]` for facts drawn from earlier turns.
- **Full-context strategy (for contract-grounded questions):** For contracts ≤ 15,000 tokens, the entire document is passed whenever a question is classified `contract` or `both` (no chunking / vector retrieval needed at MVP). This guarantees no relevant clause is missed due to retrieval error. A chunked RAG strategy is deferred until contract length limits are raised post-v1.0.
- **Conversation memory:** The classifier (`contract` / `history` / `both`, heuristic, no extra API call) determines both the system prompt *and* how much history is retrieved — the **last 10 turns** for `contract`/`both`, the **last 20 turns** for `history` (a "turn" = one user + one assistant message). History is always fetched from the DB before the new user message is written, so the classifier and model never see the in-progress question as prior history. Full history remains stored indefinitely regardless of what's sent to the model on any given turn, enabling memory-style questions ("what have I asked you so far?").
...
| Contract chat (Q&A) | Retrieval-Augmented, classification-gated: `contract`/`both` questions get full contract text + last 10 conversation turns; `history` questions get the last 20 conversation turns and **no** contract text; system prompt is matched to the classification | Free text with a mandatory attribution tag: `[Page X]` for contract facts, `[From conversation]` for conversation-sourced facts | Grounds responses strictly in the uploaded document and/or the actual prior conversation; prevents hallucination from general legal knowledge, and prevents memory-only answers from leaning on document content they weren't asked about |
...
11. **Chat uses full contract text as context whenever a question is classified `contract` or `both`** (not a chunked RAG approach, and not included at all for `history`-classified questions). This works for contracts ≤ 15,000 tokens but will require a chunking strategy if contract length limits are increased post-v1.0.
...
14. **Only a bounded recent window of conversation history is passed to the chat model each turn — not the full history, and the window size itself depends on classification.** `contract`/`both` questions get the last 10 turns; `history` questions get the last 20 turns. The chat route fetches the full session history from the DB (bounded to 40 most-recent messages, the largest any classification needs) in ascending order before the new user message is written, then trims to the classification's window. All messages remain stored indefinitely regardless of what's sent to the model on a given turn, enabling memory-style questions ("what have I asked you so far?") within that window. The query classification layer (`contract` / `history` / `both`, heuristic, no extra API call) determines both the system prompt *and* what context is retrieved — this is a real structural difference per classification, not just a prompt-preamble hint.
